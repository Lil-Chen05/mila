...
- Question-level groups use **final-checkpoint (frac=1.0) correctness** (`forced_letter==gold`),
  not `natural_pred==gold`: **13 correct / 6 incorrect**.
...
|---|---|---|
| mean answer-letter entropy (nats) | 1.129 | 0.038 |
| mean verbalized confidence | 88.2 | 93.9 |
| forced-answer accuracy | 0.53 | 0.68 |

See `fig_entropy_vs_frac.png`, `fig_confidence_vs_frac.png`, `fig_accuracy_vs_frac.png`.

**Two wrinkles, both honest gaps not bugs:**
- Accuracy is **non-monotonic**: it dips to **0.32 at frac 0.1** before
  recovering to 0.68 — a *little* reasoning underperforms *none* here. Intriguing (echoes
  "partial chain-of-thought can hurt") but well within n=19 noise; flagged, not claimed.
- **1 checkpoint** (qid15 @ frac 0.5) produced no clean answer token, so its
...
- gap peaks at **frac 0.8** (**0.502 nats**),
- endpoint gap (frac 1.0) = **-0.010 nats** (H_correct=0.041, H_incorrect=0.031) — effectively converged.
...
|---|---|---|
| correct (n=13) | 1572 | 1595 |
| incorrect (n=6) | 1595 | 1228 |
...
> **survives: correct group shows lower mean entropy in 8/8 overlapping absolute-position bins.**
...
- mean commit fraction: **correct 0.32** vs **incorrect 0.45**
- mean answer flips along the chain: **correct 2.15** vs **incorrect 2.67**
...
**Pearson r = -0.381, Spearman ρ = -0.305** (negative = the two signals agree:
...
---
*Generated by `analysis/analyze_checkpoints.py` from `results/checkpoints_20q.jsonl`.*
